...
class Agressive_Monster(sprite.Sprite):
    ...
    def update(self, platforms, playerX, playerY):
        ...
        if playerX >= self.StartX and playerX < self.StartX + self.maxWay and playerY <= self.StartY and playerY >= abs(self.StartY - self.maxUp): #проверка на наличие игрока в защищаемой зоне
            self.Triggered = True
        else:
            self.Triggered = False
