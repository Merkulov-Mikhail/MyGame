...
class Falling_Block(Platform):
    def __init__(self, x, y, EndX, EndY, Move_Speed_OX, Move_Speed_OY, Timeout): #ВАЖНОЕ УСЛОВИЕ ДЛЯ РАСПОЛОЖЕНИЯ falling, ЭТО ЧТОБЫ (КОНЕЧНАЯ КОРДИНАТА - НАЧАЛЬНАЯ КОРДИНАТА) ДЕЛИЛОСЬ БЕЗ ОСТАТКА НА ЕГО  СКОРОСТЬ (ЭТО ОТНОСИТСЯ КАК И К ОСИ "OX" ТАК И К ОСИ "ОY")
        Platform.__init__(self, x, y)
